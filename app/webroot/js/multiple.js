    ...
    return $(this).each(function(group_count){
     if(this._MultiFile) return; this._MultiFile = true;
       ...
       if(!(o.max>0) /*IsNull(MF.max)*/){
		...
		// get the value from attribute maxLengthPhoto while uploading files
		o.max = MF.E.attr('maxlengthPhoto');	
		
		
		
		// set the maximum number of uploadable files to 3
		//o.max = 3;
		
		
		
		
        if(!(o.max = 4) /*IsNull(MF.max)*/){
       ...
       MF.addToList = function( slave ){
        ...
        $('.multi').click(function(){				
			var id=$(this).attr('id');
			if(id=='VideoLinkPicture_F4'){
				$('#VideoLinkPicture_wrap').css({'overflow-y':'scroll','height':'95px'});
			}
		})
        b.click(function(){
